<script lang="ts">
	import { 
		...
		Plus, AlertCircle // Tambah icon Alert
	} from 'lucide-svelte';
...
<div class="space-y-6">
	<div class="flex flex-col md:flex-row justify-between gap-4 items-center">
		<div class="w-full md:w-auto">
			...
			<p class="text-gray-500 text-sm mt-1">Pantau status pengerjaan dan kelola pesanan masuk.</p>
		</div>
		<div class="flex gap-3 w-full md:w-auto">
			<button class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition shadow-sm text-sm font-medium">
				<Download size={18}/> <span class="hidden sm:inline">Export CSV</span>
			</button>
			<button class="flex items-center gap-2 px-4 py-2 bg-t-yellow text-dark rounded-xl hover:bg-yellow-400 transition shadow-md text-sm font-medium">
		...
		<nav class="flex gap-6 overflow-x-auto" aria-label="Tabs">
			{#each tabs as tab}
				...
				>
					{tab.label}
					{#if tab.count > 0}
						<span class="ml-2 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded-full shadow-sm">
							{tab.count}
						</span>
					{/if}
				</button>
				...
				<div class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition relative overflow-hidden group">
					...
					<div class="flex justify-between items-start mb-4 pl-2">
						<div>
							...
							<p class="text-xs text-gray-400 font-mono">{req.id} • {req.date}</p>
						</div>
						<div class="p-2 bg-gray-100 text-gray-500 rounded-lg" title="Menunggu Penentuan Harga">
							<AlertCircle size={20} />
						</div>
					</div>
					...
					<div class="space-y-3 pl-2 mb-6">
						<div>
							<p class="text-xs text-gray-400 font-bold uppercase">Item Pesanan</p>
						...
						<div class="pt-2 border-t border-dashed border-gray-200 flex items-center gap-2">
							...
							<div>
								<p class="text-[10px] text-gray-400 font-bold uppercase">Permintaan Selesai</p>
								<p class="text-sm font-bold text-gray-700">{req.requestedDeadline}</p>
							</div>
						</div>
					</div>

					<div class="flex gap-3 pl-2">
						<button class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-red-200 text-red-600 hover:bg-red-50 font-medium text-sm transition">
							<XCircle size={18} /> Tolak
						</button>
						<button class="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-t-blue text-white hover:bg-blue-600 font-bold text-sm shadow-md transition">
	...
	{:else}
		<div class="flex gap-3 mb-4">
			...
			<button class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-gray-600 text-sm font-medium flex items-center gap-2 hover:bg-gray-50">
		...
		<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
				...
				<table class="w-full text-left text-sm">
					<thead class="bg-gray-50 text-gray-500 uppercase font-bold text-[11px] border-b border-gray-100 tracking-wider">
						<tr>
							...
							<th class="px-6 py-4">Item & Model</th>
							...
							<tr class="hover:bg-gray-50 transition group">
								...
								<td class="px-6 py-4">
									<div class="flex items-center gap-2 text-gray-600">
										<Calendar size={14} class="text-t-pink"/>
										<span class="font-medium">{order.deadline}</span>
									</div>
								</td>
								<td class="px-6 py-4">
									<span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide {statusColor(order.status)}">
										{order.status}
									</span>
								</td>
								<td class="px-6 py-4 text-center">
									<div class="flex items-center justify-center gap-2">
										<button class="p-1.5 text-gray-400 hover:text-t-blue hover:bg-blue-50 rounded-lg transition" title="Lihat Detail">
											<Eye size={18}/>
										</button>
										<button class="p-1.5 text-gray-400 hover:text-t-yellow hover:bg-yellow-50 rounded-lg transition" title="Edit">
											<Pencil size={18}/>
										</button>
										<button class="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition" title="Hapus">
											<Trash2 size={18}/>
										</button>
									</div>
								</td>
			...
			<div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
				...
				<div class="flex gap-2">
					<button class="p-2 border border-gray-200 rounded-lg disabled:opacity-50 hover:bg-gray-50">
						<ChevronLeft size={16} />
					</button>
