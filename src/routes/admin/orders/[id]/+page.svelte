<script lang="ts">
    ...
    // Fungsi klik manual (Tambah param ke URL biar konsisten)
    ...
    // Fungsi Batal (Updated: Async & Clear URL First)
    async function cancelEdit() {
        await clearUrlParam(); // Tunggu URL bersih dulu
        isEditing = false;     // Baru matikan edit mode
    }

    // Fungsi Simpan (Updated: Async & Clear URL First)
    ...
    // Helper Bersihkan URL (Return Promise)
    async function clearUrlParam() {
        const newUrl = new URL($page.url);
        if (newUrl.searchParams.has('edit')) {
            newUrl.searchParams.delete('edit');
            // Kita return promise goto agar bisa di-await
            await goto(newUrl, { replaceState: true, keepFocus: true });
        }
    }

    // Effect: Hanya menyalakan edit mode jika URL memintanya
    $effect(() => {
        const urlHasEdit = $page.url.searchParams.get('edit') === 'true';
        const currentlyEditing = untrack(() => isEditing);

        if (urlHasEdit && !currentlyEditing) {
            enableEditMode();
        }
    });
</script>
...
<div class="space-y-6 pb-20">
  <div
    class="flex items-center justify-between sticky top-20 z-40 bg-t-gray/90 backdrop-blur-sm py-2"
  >
    <div class="flex items-center gap-4">
      <a
        href="/admin/orders"
        class="p-2 rounded-xl bg-gradient-to-r from-t-blue to-t-pink border border-gray-200 text-white hover:bg-gray-50 transition shadow-sm"
      >
        <ArrowLeft size={20} />
      </a>
      <div>
        <div class="flex items-center gap-3">
          ...
          <span
            class="px-3 py-1 rounded-full text-sm font-bold border flex items-center gap-1.5 {getStatusColor(
              order.status
            )}"
          >
        ...
        <p class="text-gray-500 text-sm mt-1">
          Detail spesifikasi dan status pengerjaan.
        </p>
      </div>
    </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <div class="space-y-6">
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="space-y-4">
          <div>
            ...
            <a
              href="/admin/measurements/{order.customer.id}"
              class="text-xs text-t-blue hover:underline font-mono"
            >
      ...
      <div
        class="bg-gradient-to-r from-t-blue to-t-pink rounded-2xl p-6 text-white shadow-lg relative overflow-hidden group"
      >
        <div class="relative z-10">
          ...
          <p class="text-blue-100 text-sm mb-6">
            Lihat detail ukuran badan pelanggan ini untuk proses pemotongan
            pola.
          </p>

          <a
            href="/admin/measurements/{order.customer
              .id}?ref=/admin/orders/{order.id}"
        ...
        <div
          class="absolute -right-6 -bottom-6 w-32 h-32 bg-white/10 rounded-full blur-2xl group-hover:bg-white/20 transition duration-500"
        ></div>
      </div>
    </div>
    ...
    <div class="lg:col-span-2 space-y-6">
      <div
        class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-6"
      >
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
          <p
            class="text-xs text-gray-400 font-bold uppercase mb-2 flex items-center gap-1"
          >
        ...
        <div class="bg-red-50 p-4 rounded-xl border border-red-100 relative">
          <p
            class="text-xs text-red-400 font-bold uppercase mb-2 flex items-center gap-1"
          >
          ...
          {#if isEditing}
            <input
              type="text"
              bind:value={editForm.deadline}
              class="w-full bg-white border border-red-200 rounded px-2 py-1 text-red-600 font-bold focus:ring-2 focus:ring-red-200 outline-none"
            />
          {:else}
        ...
        <div
          class="bg-green-50 p-4 rounded-xl border border-green-100 text-right md:text-left relative"
        >
          <p class="text-xs text-green-600 font-bold uppercase mb-2">
            Total Biaya
          </p>
          {#if isEditing}
            <div class="flex items-center">
              ...
              <input
                ...
                class="w-full bg-white border border-green-200 rounded px-2 py-1 text-green-700 font-bold focus:ring-2 focus:ring-green-200 outline-none"
              />
            </div>
          {:else}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
          <div>
            ...
            {#if isEditing}
              <input
                type="text"
                bind:value={editForm.clothingType}
                class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-gray-800 font-bold focus:ring-2 focus:ring-t-blue/20 outline-none"
              />
            {:else}
          ...
          <div class="md:col-span-2">
            ...
            {#if isEditing}
              <input
                type="text"
                bind:value={editForm.fabric}
                class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-gray-800 font-medium focus:ring-2 focus:ring-t-blue/20 outline-none"
              />
            {:else}
              <div
                class="flex items-center gap-2 text-gray-700 font-medium p-3 bg-gray-50 rounded-lg border border-gray-100"
              >
          ...
          <div class="md:col-span-2">
            <label
              class="block text-xs font-bold text-gray-400 uppercase mb-2 flex items-center gap-2"
            >
            ...
            {#if isEditing}
              <textarea
                ...
                rows="3"
                class="w-full bg-yellow-50/50 border border-yellow-200 rounded-lg px-3 py-2 text-gray-700 leading-relaxed focus:ring-2 focus:ring-t-yellow/50 outline-none"
              ></textarea>
            {:else}
              <div
                class="bg-yellow-50 border border-yellow-100 p-4 rounded-xl text-gray-700 italic leading-relaxed"
              >
                "{order.notes}"
              </div>
            {/if}
