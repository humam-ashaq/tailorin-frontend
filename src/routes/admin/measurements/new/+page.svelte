<script lang="ts">
    ...
    // LOGIKA BARU 1: Cek apakah ada data bawaan dari halaman Pesanan?
    $effect(() => {
        const params = $page.url.searchParams;
        // Jika ada parameter 'name' atau 'phone' di URL, isi ke form
        if (params.has('name') && !newCustomer.name) {
            newCustomer.name = params.get('name') || '';
        }
        if (params.has('phone') && !newCustomer.phone) {
            newCustomer.phone = params.get('phone') || '';
        }
    });
    ...
    function handleSave() {
        ...
        // LOGIKA BARU 2: Cek mau kembali ke mana?
        const params = $page.url.searchParams;
        const ref = params.get('ref');

        if (ref) {
            // Jika ada referensi (misal dari buat pesanan), kembali ke sana bawa data
            const returnParams = new URLSearchParams();
            returnParams.set('fromMeasurement', 'true');
            returnParams.set('name', newCustomer.name);
            returnParams.set('phone', newCustomer.phone);
            // Kembali ke halaman asal (ref)
            goto(`${ref}?${returnParams.toString()}`);
        } else {
            // Default behavior: kembali ke daftar ukuran
        ...
        <div class="flex items-center gap-4">
            <button 
                onclick={() => {
                    ...
                    if (ref) {
                        // Kalau ada ref, balik ke ref tapi bawa data yg sudah diketik (opsional, biar ga hilang)
                        const returnParams = new URLSearchParams();
                        returnParams.set('fromMeasurement', 'true'); // Penanda saja
                        returnParams.set('name', newCustomer.name);
                        returnParams.set('phone', newCustomer.phone);
                        goto(`${ref}?${returnParams.toString()}`);
                    } else {
                ...
                class="p-2 rounded-xl bg-gradient-to-r from-t-blue to-t-pink border border-gray-200 text-white hover:bg-gray-50 hover:text-t-blue transition shadow-sm"
            >
                <ArrowLeft size={20} />
            </button>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" bind:value={newCustomer.name} placeholder="Contoh: Siti Aminah" class="w-full pl-10 pr-4 py-3 bg-gray-50 border-transparent rounded-xl text-gray-800 focus:bg-white focus:ring-2 focus:ring-t-blue/20 transition"/>
                        </div>
                    </div>
                    <div>
                        <!-- svelte-ignore a11y_label_has_associated_control -->
                        ...
                        <div class="relative">
                            ...
                            <input type="tel" bind:value={newCustomer.phone} placeholder="08xxxxxxxxxx" class="w-full pl-10 pr-4 py-3 bg-gray-50 border-transparent rounded-xl text-gray-800 focus:bg-white focus:ring-2 focus:ring-t-blue/20 transition"/>
                        </div>
                    </div>
                    <div>
                        <!-- svelte-ignore a11y_label_has_associated_control -->
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <button onclick={() => newCustomer.gender = 'Pria'} class="py-2.5 rounded-xl text-sm font-bold border transition-all {newCustomer.gender === 'Pria' ? 'bg-blue-50 border-t-blue text-t-blue' : 'bg-gray-50 border-transparent text-gray-500 hover:bg-gray-100'}">Pria</button>
                            <button onclick={() => newCustomer.gender = 'Wanita'} class="py-2.5 rounded-xl text-sm font-bold border transition-all {newCustomer.gender === 'Wanita' ? 'bg-pink-50 border-t-pink text-pink-500' : 'bg-gray-50 border-transparent text-gray-500 hover:bg-gray-100'}">Wanita</button>
                        </div>
                    </div>
                    <div>
                        <!-- svelte-ignore a11y_label_has_associated_control -->
                        ...
                        <div class="relative">
                            ...
                            <textarea bind:value={newCustomer.address} placeholder="Alamat lengkap..." class="w-full pl-10 pr-4 py-3 bg-gray-50 border-transparent rounded-xl text-gray-800 focus:bg-white focus:ring-2 focus:ring-t-blue/20 transition min-h-[80px] resize-none"></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <textarea bind:value={newCustomer.notes} class="w-full bg-yellow-50/50 border border-yellow-100 rounded-xl p-4 text-sm text-gray-700 focus:ring-2 focus:ring-t-yellow/50 focus:border-t-yellow min-h-[120px]" placeholder="Catatan khusus bentuk tubuh..."></textarea>
            </div>
        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-6">
                        {#each Object.entries(section.data) as [key, value]}
                            <div class="relative">
                                <!-- svelte-ignore a11y_label_has_associated_control -->
                                <label class="block text-xs font-bold text-gray-400 uppercase mb-1.5">{formatLabel(key)}</label>
                                <div class="relative">
                                    <input type="number" bind:value={section.data[key as keyof typeof section.data]} placeholder="0" class="w-full pl-4 pr-10 py-2.5 bg-gray-50 border-transparent rounded-xl font-bold text-gray-800 focus:bg-white focus:ring-2 {section.ring} transition"/>
